...
--
-- SELECT unnest(string_to_array(FIDC, NULL)) as character,
--        ascii(unnest(string_to_array(FIDC, NULL))) as ordinal_value
-- FROM D_ESPECIFICACAO where FIDC = 'ON';
--
-- SELECT distinct unnest(string_to_array(ESPECI, NULL)) as character,
--        ascii(unnest(string_to_array(ESPECI, NULL))) as ordinal_value
-- FROM COT_HISTORICA where ESPECI = 'PNA';

--select length of string in COD_BDI in cot_historica
SELECT DISTINCT LENGTH(CODBDI) FROM COT_HISTORICA;
SELECT DISTINCT LENGTH(COD_BDI) FROM D_BDI;
DELETE FROM D_BDI;
DELETE FROM COT_HISTORICA;
